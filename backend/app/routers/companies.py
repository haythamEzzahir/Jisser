...
async def list_needs(auth: dict = Depends(verify_token)):
    ...
    result = supabase.table("company_needs").select("*").eq("company_id", company.data["id"]).order("created_at", desc=True).execute()
...
async def get_assigned_students(auth: dict = Depends(company_only)):
    ...
    if not company.data:
        return {"success": True, "data": []}
    contracts = supabase.table("contracts").select("*, student_profiles!inner(*, profiles!inner(full_name, email))").eq("company_id", company.data["id"]).in_("status", ["active", "completed"]).execute()
...
async def get_roi(auth: dict = Depends(company_only)):
    ...
    contracts = supabase.table("contracts").select("*").eq("company_id", company.data["id"]).execute()
