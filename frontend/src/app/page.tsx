...
export default function Home() {
  ...
  useEffect(() => {
    if (!loading && profile) {
      ...
      else if (profile.role === "company") router.push("/company/dashboard");
      ...
      <header className="fixed top-0 z-50 w-full border-b bg-background/80 backdrop-blur-lg">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-4">
            <Link href="/login" className="text-sm font-medium text-muted-foreground transition-colors hover:text-foreground">Connexion</Link>
            <Button asChild size="sm">
              <Link href="/register">Postuler →</Link>
            </Button>
          </div>
        </div>
      </header>
        ...
        <section className="relative overflow-hidden pt-32 pb-20">
          ...
          <div className="mx-auto max-w-5xl px-6 text-center relative">
            ...
            <p className="animate-fade-in-up mx-auto mt-6 max-w-2xl text-lg text-muted-foreground" style={{ animationDelay: "0.1s" }}>
              Jesser connecte les étudiants ambitieux aux entreprises qui investissent dans leur avenir.
            ...
            <div className="animate-fade-in-up mt-10 flex items-center justify-center gap-4" style={{ animationDelay: "0.2s" }}>
              <Button size="lg" asChild className="gap-2">
                <Link href="/register">Postuler maintenant <ArrowRight className="h-4 w-4" /></Link>
              </Button>
            ...
            <div className="grid items-center gap-12 md:grid-cols-2">
              <div className="animate-fade-in-up">
                ...
                <Button asChild className="mt-8 gap-2" variant="secondary">
                  <Link href="/register">Rejoindre comme entreprise <ArrowRight className="h-4 w-4" /></Link>
                </Button>
              </div>
        ...
        <div className="mx-auto max-w-6xl px-6">
          <div className="flex flex-col items-center justify-between gap-4 md:flex-row">
            <div className="flex items-center gap-2">
              <Image src="/logo.png" alt="Jesser" width={20} height={20} className="h-5 w-5" />
              <span className="font-heading font-bold tracking-tight">Jesser</span>
            </div>
